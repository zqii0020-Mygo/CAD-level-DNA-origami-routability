...
The failure head has one slot per class the dataset actually contains, so the
caller passes `n_classes`; see `data.ClassMap`.
...
class Heads(nn.Module):
    ...
    def __init__(self, dim: int, hidden: int = 128, n_classes: int = N_CLASSES):
        super().__init__()
...
class GraphMLP(nn.Module):
    ...
    def __init__(self, graph_dim: int, hidden: int = 128, n_classes: int = N_CLASSES):
        super().__init__()
        self.encode = nn.Sequential(
            nn.Linear(graph_dim, hidden), nn.ReLU(),
            nn.Linear(hidden, hidden), nn.ReLU(),
        )
        self.heads = Heads(hidden, hidden, n_classes)
...
class HeteroGNN(nn.Module):
    def __init__(self, in_dims: dict[str, int], graph_dim: int,
                 hidden: int = 64, layers: int = 3, n_classes: int = N_CLASSES):
        super().__init__()
        ...
        self.heads = Heads(2 * hidden * len(NODE_TYPES) + graph_dim, 128, n_classes)
